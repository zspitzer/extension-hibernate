...
public class HibernateORMSession implements ORMSession {
		...
		public Session getSession( PageContext pc ) {
			if ( s == null || !s.isOpen() ) {
				if ( logger.isDebugEnabled() )
					logger.atDebug().log( "Opening new session" );
	 ...
	 *
	 * There's some duplication with the {@link #close(PageContext)} and {@link #close(PageContext, String)} methods here... May merge `close()` with
	 * `closeAll()` in the future.
	 *
	 * @param pc
	 *           Lucee PageContext object.
	 */
	 ...
	 *
	 * Hibernate v5.3+ requires JPA syntax for positional parameters in HQL queries. (i.e. `?1, ?2, ?3` parameter instead of `?, ?, ?`.) This conversion
	 * method will ensure each positional parameter has an index number to match.
	 *
